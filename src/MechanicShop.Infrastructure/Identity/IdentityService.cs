    ...
    IAuthorizationService authorizationService) : IIdentityService
{
    public async Task<Result<AppUserDto>> AuthenticateAsync(string email, string password)
    {
        ...
        return new AppUserDto(user.Id, user.Email!, await manager.GetRolesAsync(user), await manager.GetClaimsAsync(user));
    }
    ...
    public async Task<bool> AuthorizeAsync(string userId, string? policyName)
    {
        ...
        var principal = await userClaims.CreateAsync(user);
    ...
    {
         var user = await manager.FindByIdAsync(userId) ?? throw new InvalidOperationException(nameof(userId));
    ...
    public async Task<bool> IsInRoleAsync(string userId, string role)
    {
       ...
       return user != null && await manager.IsInRoleAsync(user, role);
    }
}